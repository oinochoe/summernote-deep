...
window.CodeMirror = (function() {
    ...
    function updateSelection(cm) {
        ...
        if (cm.options.moveInputWithCursor) {
            ...
            display.inputDiv.style.top =
                Math.max(0, Math.min(display.wrapper.clientHeight - 10, headPos.top + lineOff.top - wrapOff.top)) +
                'px';
            display.inputDiv.style.left =
                Math.max(0, Math.min(display.wrapper.clientWidth - 10, headPos.left + lineOff.left - wrapOff.left)) +
                'px';
        }
    }
    ...
    function updateSelectionRange(cm) {
        ...
        function drawForLine(line, fromArg, toArg) {
            ...
            iterateBidiSections(getOrder(lineObj), fromArg || 0, toArg == null ? lineLen : toArg, function(
                from,
                to,
                dir,
            ) {
        ...
        if (dx && !gecko && !opera && wheelPixelsPerUnit != null) {
            if (dy)
                setScrollTop(
                    ...
                    Math.max(
                        0,
                        Math.min(scroll.scrollTop + dy * wheelPixelsPerUnit, scroll.scrollHeight - scroll.clientHeight),
                    ),
                );
            setScrollLeft(
                cm,
    ...
    function markText(doc, from, to, options, type) {
        ...
        if (
            posLess(to, from) ||
            (posEq(from, to) && type == 'range' && !(options.inclusiveLeft && options.inclusiveRight))
        )
            return marker;
